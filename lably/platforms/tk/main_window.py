...
class TKMainWindow(BaseMainWindow):
    def __init__(self) -> None:
        ...
        notebook.add(file_tab, text="File Picker")

        self.file_path_var = tk.StringVar()
        self.reverse_image = tk.IntVar()
        ttk.Label(file_tab, text="Select a file:").pack(pady=5)
        ttk.Button(file_tab, text="Browse...", command=self.browse_file).pack(pady=5)
        ttk.Entry(file_tab, textvariable=self.file_path_var, width=50).pack(pady=5)
        ttk.Checkbutton(
            file_tab, text="Reverse", variable=self.reverse_image,
            onvalue=1, offvalue=0,
        ).pack(pady=5)
        ttk.Button(file_tab, text="Print", command=self.print_file).pack(pady=5)

        # Text Field Tab
        text_tab = ttk.Frame(notebook)
        notebook.add(text_tab, text="Text Field")

        self.reverse_barcode = tk.IntVar()
        self.text_entry = tk.Text(text_tab, height=10, width=50)
        self.text_entry.pack(pady=5)
        ttk.Checkbutton(
            text_tab, text="Reverse", variable=self.reverse_barcode,
            onvalue=1, offvalue=0,
        ).pack(pady=5)
        ttk.Button(text_tab, text="Print", command=self.print_barcode).pack(pady=5)
    ...
    def print_file(self) -> None:
        ...
        try:
            ...
            messagebox.showerror("Error Printing", str(ex))
        except ValueError as ex:
            messagebox.showerror("Error Processing", str(ex))
        except Exception as ex:
            messagebox.showerror("Other Issue", str(ex))
    ...
    def _show_about(self) -> None:
        ...
        win.title("About Lably")
        win.resizable(False, False)
        win.grab_set()  # make it modal

        ttk.Label(win, text="Lably", font=("", 16, "bold")).pack(pady=(20, 4))
        ttk.Label(win, text=f"Version {version}").pack()
        ttk.Label(win, text="A GUI for printing labels on DYMO Bluetooth printers.").pack(
            pady=(8, 0), padx=20,
        )
        ttk.Separator(win, orient="horizontal").pack(fill="x", padx=20, pady=12)
        ttk.Label(win, text="Developer").pack()
